...
let counts = 0;
(async () => {
  const start = async (files_: string[], WIDTH: number, HEIGHT: number) => {
    const POW = WIDTH * HEIGHT;

    let files = <string[]>files_
      .map((file) => {
        ...
        if (fileName.includes("-") && !emojiIndex) return void 0;
        return file;
      })
      .filter((file) => file);
  ...
  await start(await asyncFiles("assets/32x32/*.png"), 7, 199);
  await start(await asyncFiles("assets/discordEmojis-32x32/*.png"), 8, 11);
})();
